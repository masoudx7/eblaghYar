...
async function sendKavenegarSms(
  ...
  if (!apiKey) {
    ...
    console.log(`💡 جهت ارسال واقعی پیامک، متغیر KAVENEGAR_API_KEY را در تنظیمات وارد نمایید.`);
    console.log(`========================================\n`);
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (attempt < maxRetries - 1) {
        // For model switches during high-demand/503, switch quickly to the fallback model
...
function cleanAndParseJSON(raw: string): any {
  ...
  try {
    ...
    // Attempt to locate JSON object within string
...
async function startServer() {
  ...
  // Middleware to support base64 uploads of PDFs and images
  ...
  app.post("/api/auth/send-otp", async (req, res) => {
    try {
      ...
      return res.json({
        ...
        // Provide devCode only when running in simulation/dev mode without API key
  ...
  app.post("/api/auth/verify-otp", async (req, res) => {
    try {
      ...
      // Successful verification -> Generate Session Token
      otpStore.delete(phoneNumber);
  ...
  app.get("/api/auth/me", (req, res) => {
    ...
    return res.json({
      ...
      user: {
        ...
        role: "کاربر حقیقی",
        loggedInSince: session.createdAt,
      },
    });
  });
...
`;
  ...
  app.post("/api/generate-defense-draft", async (req, res) => {
    try {
...
`;
  ...
  // Chat with legal assistant about this notice (with support for focused section queries)
